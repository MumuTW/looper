...
5. **`internal/api/loops.go`** — the loop route builders and handlers that are not already in `loop_retry_discard.go` / `loop_logs_stream.go`: `loopsListResponse`, `maxLoopsListLimit`, `loopResponse`, `loopLogsResponse`, `loopLogsRunResponse`, `loopLogsAgentPayload`, `retryLoopRequest`, `retryLoopResponse`, `buildLoopsRouteResponse`, `parseLoopsListOptions`, `buildLoopRouteResponse`, `retryLoop`, and the loop start/stop/retry/discard route entry points that remain in `handler.go` today. `retryLoop` currently lives in `handler.go` (not `loop_retry_discard.go`, which holds only the worktree/discard helpers), so it moves here with the rest of the loop route/response layer; the discard path already extracted into `loop_retry_discard.go` stays put. (~700 lines moved)

   The loop-log implementation that is still in `handler.go` moves into the existing **`internal/api/loop_logs_stream.go`** (not `loops.go`), because that file already owns the combined streaming path and already calls `buildLogsStateForRun`. Specifically, the loops slice also relocates: the follow/streaming route functions `isFollowLoopLogsRequest`, `streamLoopLogsRoute`, `streamLoopLogs`, `newLoopLogsFollowErrorEvent`, `queryBool`, `writeSSEEvent`, `loopLogsStreamState`, `shouldDefaultLoopLogsStreamToStderr`, `appendedLogChunk`, `shouldTerminateLoopLogsFollow`, `shouldTerminateLoopLogsFollowBeforeChunk` (`handler.go:3887-4095`); the log snapshot builders `buildLoopLogsResponse`, `buildLogsResponseForRun`, `buildLogsStateForRun` (`handler.go:6530-6621`, the latter already called by `loop_logs_stream.go`); and the persisted-agent-log helpers `maxPersistedAgentLogReadBytes`, `agentOutputPayload`, `decodeAgentOutput`, `materializeAgentOutput`, `readAgentOutputLog`, `isPathWithinDirectory` (`handler.go:7434-7508`, the last shared with `loop_logs_stream.go`). The generic JSON helpers `parseJSONObject`, `readObject`, `readObjectString`, `readStringMap`, `readStringAny`, `firstNonEmptyString`, `stringPtrOrNil` (`handler.go:7510-7568`) are **not** moved — they are used by runs/workers/pull-requests as well, so they stay in `handler.go` per the shared-helper rule in *Risks*. `buildRunLogsResponse` is run-route-only and moves with `runs.go` (see item 8), calling `buildLogsResponseForRun` across files in the same package.
6. **`internal/api/projects.go`** — `projectsListResponse`, `projectResponse`, `projectValidationResponse`, `discoveryResponse`, `createProjectResponse`, `projectService`, `buildProjectsRouteResponse`, `buildProjectRouteResponse`, `buildProjectDiscoverResponse`, `projectDiscoverResponse`, plus the project mutation and serialization helpers that those route builders call and that otherwise remain stranded in `handler.go`: `createProjectRequest`, `updateProjectStringField` (with its `UnmarshalJSON` method), `updateProjectRequest`, `updateProjectField`, `buildCreateProjectResponse` (called by `buildProjectsRouteResponse`), `buildUpdateProjectResponse` (called by `buildProjectRouteResponse`), `serializeDiscovery`, `serializeProject` (called by the create/update/discover paths), `serializeProjectValidation`, `resolveProjectProviderKind` (sole caller is `serializeProject`), `stringMetadataValue` (sole caller is `resolveProjectProviderKind`), and `deriveProjectIDFromRepoPath` (the `handler.go` copy at ~line 7963, sole caller is `buildCreateProjectResponse`). The shared project-metadata helpers `parseProjectMetadata` and `stringMetadataPtr` (the `map[string]any` overload at ~line 7928) are used by loops/workers too, so they stay in `handler.go` per the shared-helper rule in *Risks*; `normalizeOptionalString` is likewise shared and stays. (~430 lines moved)
...
After all slices land, `handler.go` should contain only: package/imports, the `const`/`var` block of shared constants and regexps, `RuntimeState`, `activeRunExecutionVerifier`, `Context`, `PullRequestTarget`, `TakeoverResult`, `Handler`, `effectiveConfig`, `NewHandler`, the lock helpers, `ServeHTTP`/`serveHTTP` (the router), `apiError`/`asAPIError`/`internalServerError`/`assertMethod`/`isMutatingHTTPMethod`/`isAdmissionExemptMutationPath`/`admissionMutationDenial`/`admissionStateString`, `authorizeRequest`/`isDirectLoopbackConfigMutation`/`isLoopbackRemoteAddr`/`hasForwardingProxyHeaders` (the shared authorization helpers, kept beside the auth code that calls them), `normalizePath`, `writeSuccess`/`writeError`/`writeJSON`, `generateRequestID`, and the small shared helpers (`hasValue`, `homeDirOrEmpty`, `currentLooperdTarget`, `mapKeys`, `parsePositiveInt64`, `parseJSONObject`, `readObject`, `readObjectString`, `readStringMap`, `readStringAny`, `firstNonEmptyString`, `stringPtrOrNil`, `parseProjectMetadata`, `stringMetadataPtr`, `normalizeOptionalString`, `derefString`). The generic JSON helpers and project-metadata helpers in that list are used by ≥2 route groups, so they stay as shared plumbing per the rule in *Risks*. Target: under ~1,200 lines.
