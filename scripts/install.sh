...
log "The looperd daemon is a separate binary you install and run yourself:"
log "  download looperd-$target.tar.gz from the same release and put it on your PATH"
log ""
log "Next steps:"
log "  1. Install looperd from the same release (or: go build -o looperd ./cmd/looperd)"
log "  2. Run: looper init   # writes ~/.looper/config.toml; then edit it"
log "  3. Run: looperd"
log "  4. Register a project: looper project add /path/to/repo  (in another shell)"
log "  5. Check it: looper status"
log "  6. Control loops: looper stop|close|takeover|handback|terminate|retry|start|pause|respond"
