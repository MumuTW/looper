...
- `looper upgrade backup` explicitly asks the running daemon to create a rollback bundle. It uses SQLite's online backup operation rather than copying a live WAL database file, then records the copied config, CLI, daemon, and database snapshot with SHA-256 checksums in `manifest.json`. New bundles also record the original config/DB/binary destinations required for a fail-closed restore; older v1 bundles remain verifiable but cannot be used for automatic restore because their targets are unknown.
...
- `looper upgrade restore-preflight --bundle <directory>` verifies a v2 bundle's recorded restore targets and fails closed unless `lsof` can prove the original config, SQLite database/WAL/SHM, and matching binaries are unused. It only reports readiness; it does not restore files or stop a daemon.
- `looper upgrade restore --bundle <directory> --confirm` is the explicit config/database rollback action. It verifies the v2 bundle again, requires `lsof` to prove the config and SQLite targets unused before and after taking the database exclusion lock, then restores only the config and SQLite snapshot (not copied binaries). The restore stages replacements alongside their targets and journals the rename transaction so an interrupted config/database pair is recovered on the next confirmed restore. Point the release `current` symlink at the matching staged binary release separately, then restart and run `verify-start`.
- Migration `0022_durable_payload_baseline` is a compatibility barrier for durable JSON checkpoints, events, pull-request snapshots, and agent retry snapshots. A pre-0022 binary must restore a matching backup rather than open a database after this marker; future incompatible payload changes require a later barrier migration. The barrier relies on `ValidateCompatibility` to refuse boot, so it cannot stop binaries built before that check existed (commit `62251ff`) — those ignore unknown applied IDs and start normally. The upgrade preflight's build-identity gate is what prevents activating such pre-validation binaries; do not bypass preflight and point a release symlink at one by hand.
