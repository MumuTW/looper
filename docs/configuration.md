...
`reproducer.enabled` turns on the Reproducer Role. It is `false` by default, matching `hitl.enabled`, because enabling it changes what "done" means for every bug in a project.
...
When enabled, an Issue whose Triage Report classifies it as a `bug` is routed to Reproducer instead of straight to Planner. Reproducer opens the branch Planner will later adopt, runs its own agent session there, and asks for a failing reproduction plus a `.looper/reproduction.json` manifest naming the command that runs it. The daemon then executes that command through the same sandbox as `defaults.validationCommands` and accepts the reproduction only if it is observed to fail; a command that passes immediately is rejected. On acceptance the reproduction is committed and a `reproduction.recorded` event is persisted with the command, the commit SHA, and a SHA-256 hash of every reproduction file.

Worker and Fixer then require that recorded command to pass, and re-check every recorded file's hash, in addition to the unchanged `defaults.validationCommands` suite. A deleted or modified reproduction file fails the run with a distinct reason (`reproduction_test_modified` / `reproduction_test_missing`) rather than as a generic validation failure.

If Reproducer cannot make the bug fail, it writes a `reproduction.unreproducible` record describing what it attempted, what it observed instead, and what information is missing, and parks the Planner loop in `awaiting_human`. That is a decision, not a failure: no attempt counter moves and no retry is scheduled. Answering the loop through `POST /api/v1/loops/{seq}/respond` either waives the reproduction and lets Planner proceed, or leaves the Issue stopped.

Non-bug classifications, and Issues discovered by label/assignee with no Triage Report at all, are untouched: no Reproducer work, and no change to any existing gate. The field is restart-bound.
