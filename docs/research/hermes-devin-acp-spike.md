...
- **Delete them six months from now — what breaks?** The protocol, token,
  ...
  They also pin the bare-JSON parser fallback, which is not documented
  upstream, so deleting them loses the only record of that behavior.
...
tested in both directions against a local Hermes profile. **Recall works;
tool-driven writes do not.**
...
### The tool gap is a no-go, not a patch to carry

The explored MCP route is deliberately not shipped. The only observed way to
choose an approval was to correlate a vendor `_meta` tool name with a later
vendor `session/request_permission` by tool-call ID. That backend-supplied
metadata is not an authority independent of the action it asks Hermes to
approve, so an allow-list would merely infer trust from the same ACP output it
is supposed to constrain. No repository state, user confirmation, or Hermes
structured authority exists to make that approval safe.

The narrower and more honest outcome is to keep the stock deny behavior:
**treat memory as read-mostly** on this backend. Hermes recalls and reasons
over `memories/MEMORY.md` normally; writes should be made by a non-ACP Hermes
session or by editing the file directly. A `memory`-toolset session on this
backend can claim success without persisting anything. A future integration
would need an explicit authority contract from the ACP provider, rather than
another inference layer over its wire metadata.
...
- **The free tier rate-limits by message count, on a rolling window.**
  ...
  the same quota. Because the window is rolling, retrying while limited keeps
  renewing it — back off and leave it alone rather than polling.
...
Go for interactive/experimental use of the unpatched Hermes-on-Devin
compatibility workaround, with memory treated as read-only: recall works,
but write attempts require a non-ACP Hermes session or a direct edit.

The tool blocker remains. Devin's ACP server owns the tool loop, so Hermes's
prompt-level `<tool_call>` contract is not enough to make Hermes tools work;
this research does not carry a patch that weakens the stock permission deny.
...
version-pinned integration that any release on either side may break.
