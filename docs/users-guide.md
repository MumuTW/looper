...
That path must be the repository root — the directory containing `.git`. For advanced registration, `POST /api/v1/projects` accepts the same `repoPath` plus optional `id`, `baseBranch`, and `worktreeRoot` fields. The dashboard only lists registered projects; it does not create them. A `provider` binding cannot be set through the API or the dashboard — it is file-managed, so declare it in `[[projects]]` and restart the daemon.
