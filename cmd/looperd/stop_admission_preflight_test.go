...
func TestStopLoopDoesNotMutateWhenOwnershipPreflightLookupFails(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "run", dropTable: "runs", wantError: "load latest run before halt"},
		{name: "execution", dropTable: "agent_executions", withRun: true, wantError: "load latest agent execution before halt"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			if _, err := stopLoop(ctx, services, loop.ID, "Stopped by test", func() time.Time { return now }, nil, nil); err == nil {
				t.Fatal("stopLoop() error = nil, want preflight lookup failure")
			} else if !strings.Contains(err.Error(), tc.wantError) {
				t.Fatalf("stopLoop() error = %v, want %q", err, tc.wantError)
			}
