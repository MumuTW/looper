...
func TestMarkdownFooterReplacesHistoricalUpstreamLinkedFooter(t *testing.T) {
	s := testStamper()
	historical := "Body\n\n" + Marker + "\n<sub>Generated by [Looper](" + UpstreamRepoURL + ") 0.0.0-dev · runner=reviewer · agent=opencode</sub>"
	got := s.Markdown(historical, "worker", ChannelIssueComment)
	if strings.Contains(got, UpstreamRepoURL) {
		t.Fatalf("historical upstream footer was not removed: %q", got)
	}
	if strings.Count(got, Marker) != 1 {
		t.Fatalf("duplicate marker after replacing historical upstream footer: %q", got)
	}
}
