...
// this file ends here — so takeover succeeding is precisely what triggers deletion
// of the worktree it just handed to the human (#162, re-entering through the door
// #177 opened). The in-memory loop these callers hold was read before takeover
// committed, so it is not the authority; the row is. A hold refuses the removal
// before any timestamp is written: nothing was attempted, so the pair stays empty
// and the refusal is surfaced on the run's outcome instead.
//
