...
func fixerAgentSideFetchContract(repo string, prNumber int64, detail *checkpointDetail, fixItems []FixItem) string {
	...
	if strings.Contains(prURL, "/pulls/") {
		return strings.Join([]string{
			...
			"Before editing and again before final conclusions or pushing, fetch the live PR from Forgejo using `GET /api/v1/repos/{owner}/{repo}/pulls/{number}` and validate `head.sha` equals the seeded `head_sha`, `base.ref` equals the seeded `base_ref` when present, and state/draft status match the seed. Fail fast on drift. Prefer the live PR author login from `user.login` whenever it is non-empty; use the seeded `pr_author` only when the live provider omits author login. You need this identity before classifying any review-thread comment as a reviewer suggestion versus an author design clarification.",
