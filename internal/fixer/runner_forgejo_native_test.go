...
func TestBuildFixerPromptUsesForgejoSeedAndFetchContract(t *testing.T) {
	...
	prompt, _ := buildFixerPrompt("project_1", customInstructionConfig(nil), "acme/looper", 42, detail, []FixItem{{ID: "fix-1", URL: "https://code.forgejo.example/acme/looper/pulls/42/files#diff-1"}}, config.DefaultDisclosureConfig(), "opencode", "openai/gpt-5.5")
