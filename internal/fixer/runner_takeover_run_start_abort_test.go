...
// theirs. beginLoopRun is the boundary that turns the swallowed refusal back into
// an abort.
func TestProcessClaimedItemAbortsWhenTakeoverWinsTheRunStartRace(t *testing.T) {
	...
	if err := fixture.repos.Loops.UpsertChangingHumanHold(ctx, storage.LoopRecord{
		ID: loopID, Seq: 77, ProjectID: projectID, Type: "fixer",
	...
	queue := storage.QueueItemRecord{
		ID: "queue_takeover_run_start", ProjectID: &projectID, LoopID: &loopID, Type: "fixer",
		TargetType: "pull_request", TargetID: target, Repo: &repo, PRNumber: &prNumber,
		DedupeKey: "fixer:acme/looper:77", Priority: storage.QueuePriorityFixer, Status: "running",
		AvailableAt: nowISO, StartedAt: stringPtr(nowISO), Attempts: 1, MaxAttempts: 5,
	...
	runner := New(Options{DB: fixture.coordinator.DB(), Repos: fixture.repos, Git: git, GitHub: github, Logger: fixture.logger, Now: fixture.now})
	...
	// found nothing to do".
	if result.Summary != fixerRunStartHeldSummary {
		t.Fatalf("ProcessClaimedItem() summary = %q, want the run-start hold %q", result.Summary, fixerRunStartHeldSummary)
	}
