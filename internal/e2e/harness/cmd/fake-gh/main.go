...
type state struct {
	...
	//
	//   The opt-in real-GitHub contract test validates create, duplicate rejection,
	//   and forced field replacement against a provisioned sandbox. Normal CI does
	//   not run that mutating contract, so upstream drift is detected only when an
	//   operator enables LOOPER_E2E_REAL_GH=1.
