...
func (s *Service) UpdateProject(ctx context.Context, identifier string, input UpdateInput) (storage.ProjectRecord, error) {
	...
	if input.GatekeeperTrust.Set {
		...
		currentTrust := effectiveGatekeeperTrust(s.currentConfig(), updated.ID)
		if gatekeeperTrustRank(target) <= gatekeeperTrustRank(currentTrust) {
			return storage.ProjectRecord{}, ProjectValidationError{Message: fmt.Sprintf("gatekeeperTrust must advance beyond %s; %s is not a promotion", currentTrust, target)}
		}
		roles, ok := metadata["roles"].(map[string]any)
		if !ok {
		...
		gatekeeper["trust"] = string(target)
