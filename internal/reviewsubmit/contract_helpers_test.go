...
// submitFromHarness drives Submit the way the CLI does: the config file is
// loaded once, the payload comes from a file standing in for the agent's
// stdin, and cwd is the prepared checkout.
func submitFromHarness(t *testing.T, configPath, payloadPath, cwd, event, commitID string, stdout, stderr io.Writer) error {
	t.Helper()
	loaded, err := config.LoadFile(config.LoadFileOptions{Args: []string{"--config", configPath}, LookupEnv: os.LookupEnv})
	if err != nil {
	...
	return Submit(context.Background(), loaded.Config, Request{
		Repo:     "acme/looper",
		PRNumber: 42,
		Event:    event,
		CommitID: commitID,
