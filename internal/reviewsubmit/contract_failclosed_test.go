...
func TestReviewSubmitOrchestrationFailsClosedOnBaseHeadMismatch(t *testing.T) {
	...
	stdout := &bytes.Buffer{}
	stderr := &bytes.Buffer{}
	err := submitFromHarness(t, configPath, payloadPath, repo, "COMMENT", strings.Repeat("a", 40), stdout, stderr)
	if err == nil {
		t.Fatal("Submit() error = nil, want base/head mismatch fail-closed")
	}
...
func TestReviewSubmitOrchestrationFailsClosedWhenRemoteOversizedAndLocalUnavailable(t *testing.T) {
	...
	stdout := &bytes.Buffer{}
	stderr := &bytes.Buffer{}
	err := submitFromHarness(t, configPath, payloadPath, repo, "COMMENT", headSHA, stdout, stderr)
	if err == nil {
		t.Fatal("Submit() error = nil, want fail closed when authority unavailable")
	}
...
func TestReviewSubmitOrchestrationRetryDoesNotDuplicateAfterAuthorityRecovery(t *testing.T) {
	...
	stdout := &bytes.Buffer{}
	stderr := &bytes.Buffer{}
	firstErr := submitFromHarness(t, configPath, payloadPath, brokenRepo, "COMMENT", headSHA, stdout, stderr)
	if firstErr == nil {
		t.Fatal("first Submit() error = nil, want validation failure before publish")
	}
	if _, statErr := os.Stat(submitLog); !os.IsNotExist(statErr) {
		data, _ := os.ReadFile(submitLog)
		if len(bytes.TrimSpace(data)) > 0 {
	...
	stdout.Reset()
	stderr.Reset()
	if err := submitFromHarness(t, configPath, payloadPath, repo, "COMMENT", headSHA, stdout, stderr); err != nil {
		t.Fatalf("recovery Submit() error = %v\nstderr=%s", err, stderr.String())
	}
