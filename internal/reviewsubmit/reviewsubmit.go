// Package reviewsubmit publishes a validated reviewer review to a pull
// request.
//
// The reviewer agent never holds provider credentials: the daemon mints a
// trusted Unix-socket proxy that re-execs the looper binary with the captured
// tokens, and that child calls Submit. Everything here therefore treats the
// payload and the flags as agent-controlled input and re-derives publish
// authority (head/base SHAs, hold labels, review requests, diff anchors) from
// the forge and from daemon storage before mutating anything.
...
// Request is one review publication attempt. Every field originates in the
// argv the trusted review proxy execs, so none of it is authority on its own.
type Request struct {
	Repo     string
	PRNumber int64
	// Event is the requested review event: COMMENT, APPROVE, or
	// REQUEST_CHANGES.
	Event    string
	CommitID string
	// CleanReviewEvent and BlockingReviewEvent are the daemon-bound policy the
	// proxy injects; empty means the configured reviewer policy applies.
	CleanReviewEvent    string
	BlockingReviewEvent string
	ReviewerManual      bool
	ReviewerRunID       string
	// Payload is the raw review JSON the agent wrote to stdin.
	Payload []byte
	CWD     string
}
...
// Submit validates and publishes one reviewer review. cfg is the already
// resolved configuration — for a trusted proxy child that is the daemon's
// snapshot, not a locally loaded file — and stdout receives the success
// envelope while stderr receives structured diagnostics.
func Submit(ctx context.Context, cfg config.Config, req Request, stdout, stderr io.Writer) error {
	repo := req.Repo
	prNumber := req.PRNumber
	event, err := validateReviewSubmitEvent(req.Event)
	if err != nil {
		return err
	}
	...
	policy, err := effectiveReviewSubmitPolicy(
		...
		req.CleanReviewEvent,
		req.BlockingReviewEvent,
	)
	if err != nil {
		return err
	}
	...
	gateway, err := reviewSubmitGatewayForConfig(cfg, repo, cwd, diagnosticWriter)
	if err != nil {
		return err
	}
	...
	requestBypass, err := trustedReviewRequestSubmitBypass(ctx, cfg, repo, prNumber, commitID, req.ReviewerRunID)
	if err != nil {
		return err
	}
	if err := validateReviewerReviewSubmitHold(ctx, cfg, repo, prNumber, req.ReviewerManual, req.ReviewerRunID, detail.Labels); err != nil {
		return err
	}
	...
	if err != nil {
		if canSubmitWithoutAnchorValidation(err, payload.Comments) {
			...
			freshLabels, err := validateLatestReviewerReviewSubmitPublication(ctx, gateway, cfg, repo, prNumber, commitID, detail.BaseSHA, req.ReviewerManual, req.ReviewerRunID, cwd)
			if err != nil {
				return err
			}
...
func trustedReviewRequestSubmitBypass(ctx context.Context, cfg config.Config, repo string, prNumber int64, headSHA string, reviewerRunID string) (bool, error) {
	dbPath := strings.TrimSpace(cfg.Storage.DBPath)
	if dbPath == "" {
		return false, nil
	}
	...
	runID := strings.TrimSpace(reviewerRunID)
	if runID != "" {
