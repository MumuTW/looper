...
func TestParsePullRequestRef(t *testing.T) {
	...
	repo, prNumber, err := ParsePullRequestRef(" acme/looper#42 ")
	if err != nil || repo != "acme/looper" || prNumber != 42 {
		t.Fatalf("ParsePullRequestRef() = %q, %d, %v; want acme/looper, 42, nil", repo, prNumber, err)
	}
	for _, value := range []string{"acme/looper", "#42", "acme/looper#0", "acme/looper#abc", "acme/looper#1#2"} {
		if _, _, err := ParsePullRequestRef(value); err == nil {
			t.Fatalf("ParsePullRequestRef(%q) error = nil, want rejection", value)
		}
	}
}
...
func TestEffectiveReviewSubmitEventDoesNotFetchUserForComment(t *testing.T) {
	...
	got, err := effectiveReviewSubmitEvent(context.Background(), io.Discard, gh, "acme/looper", 42, "COMMENT", "reviewer", "")
	if err != nil {
		t.Fatalf("effectiveReviewSubmitEvent() error = %v", err)
	}
	if got != "COMMENT" {
