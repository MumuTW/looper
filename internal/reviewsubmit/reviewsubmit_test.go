...
func TestEffectiveReviewSubmitEventDowngradesSelfAuthoredApproval(t *testing.T) {
	...
	got, err := effectiveReviewSubmitEvent(context.Background(), stderr, gh, "acme/looper", 42, "APPROVE", "reviewer", "")
	if err != nil {
...
func TestEffectiveReviewSubmitEventKeepsApprovalForDifferentAuthor(t *testing.T) {
	...
	got, err := effectiveReviewSubmitEvent(context.Background(), stderr, gh, "acme/looper", 42, "APPROVE", "octocat", "")
	if err != nil {
		t.Fatalf("effectiveReviewSubmitEvent() error = %v", err)
	}
	if got != "APPROVE" {
...
func TestEffectiveReviewSubmitEventDoesNotFetchUserForComment(t *testing.T) {
	...
	got, err := effectiveReviewSubmitEvent(context.Background(), stderr, gh, "acme/looper", 42, "COMMENT", "reviewer", "")
	if err != nil {
		t.Fatalf("effectiveReviewSubmitEvent() error = %v", err)
	}
	if got != "COMMENT" {
...
func TestWrapReviewSubmitErrorSurfacesContentSafetyRecoveryGuidance(t *testing.T) {
	...
	err := wrapReviewSubmitError(stderr, "acme/looper", 42, "COMMENT", "abc123", payload, "submit validated PR review", outboundguard.Validate(outboundguard.Field{Name: "inline review comment 1 path", Text: secretPath}))
	if err == nil {
...
func TestValidateLatestReviewerReviewSubmitPublicationRejectsHeadAndBaseDrift(t *testing.T) {
	...
	_, err := validateLatestReviewerReviewSubmitPublication(context.Background(), gh, config.Config{}, "acme/looper", 42, head, base, false, "", "/repo")
	...
	labels, err := validateLatestReviewerReviewSubmitPublication(context.Background(), gh, config.Config{}, "acme/looper", 42, head, base, false, "", "/repo")
	if err != nil {
...
// TestRunForwardsWholeArgvToTheProxy covers the branch that runs in the agent's
// own process: when a trusted review socket is configured, nothing is published
// locally and the argv reaches the daemon unchanged. Rebuilding the argv from
// the parsed options here — or trimming a leading element from it — would hand
// the proxy a command the agent never typed, and the proxy validates argv.
func TestRunForwardsWholeArgvToTheProxy(t *testing.T) {
	argv := []string{"review", "submit", "acme/looper#42", "--event", "COMMENT", "--commit-id", "abc123"}

	// Not t.TempDir: its path is long enough to exceed the platform limit on a
	// unix socket address.
	socketDir, err := os.MkdirTemp("", "proxy")
	if err != nil {
	...
	go func() {
		...
		received <- request
		_, _ = conn.Write([]byte(`{"exitCode":0}`))
	}()

	t.Setenv(forge.TrustedReviewSockEnv, socket)
	err = Run(context.Background(), Options{
		Argv:     argv,
		PRRef:    "acme/looper#42",
		Event:    "COMMENT",
		CommitID: "abc123",
		CWD:      "/work/repo",
		Stdin:    strings.NewReader(`{"body":"x"}`),
		Stdout:   &bytes.Buffer{},
		Stderr:   &bytes.Buffer{},
	})
	if err != nil {
	...
	got := <-received
	if strings.Join(got.Argv, " ") != strings.Join(argv, " ") {
		t.Fatalf("proxy argv = %#v, want %#v", got.Argv, argv)
	}
	if string(got.Stdin) != `{"body":"x"}` {
		t.Fatalf("proxy stdin = %q, want the review payload", got.Stdin)
	}
