...
func TestClearTimeoutProgressSupersedesInterruptedObservation(t *testing.T) {
	t.Parallel()
	repos := mergeFixture(t)
	seedRunWithCheckpoint(t, repos, "run_timeout_observing", `{"execution":{"status":"timeout_observing","progressBeforeTimeout":{"headSha":"before"},"progressSnapshotError":"capture interrupted"},"work":{"title":"keep"}}`)
	...
	if strings.Contains(got, "timeout_observing") || strings.Contains(got, "progressBeforeTimeout") || strings.Contains(got, "progressSnapshotError") {
