...
func FinalizeWorkerSuccess(ctx context.Context, db *sql.DB, input WorkerSuccessFinalizationInput) error {
	...
	return WithTransaction(ctx, db, nil, func(tx *sql.Tx) error {
		...
		// applies: while `looper takeover` holds the loop, its status and
		// schedule belong to the human. The run and the queue claim are this
		// transaction's own outcome and still commit; only the loop status
		// write is dropped. Attempting it instead would be refused by the
		// human-hold guard on Loops.Upsert and would fail a finalization that
		// otherwise succeeded.
