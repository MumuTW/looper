...
// no prior item to derive from. A zero seed publishes nothing.
...
func RequeueStaleRunLoop(ctx context.Context, db *sql.DB, input StaleRunRequeueInput) (StaleRunRequeueResult, error) {
	...
	return WithTransactionValue(ctx, db, nil, func(tx *sql.Tx) (StaleRunRequeueResult, error) {
		...
		// Same rule the runners' updateLoop and FinalizeWorkerSuccess apply:
		// while `looper takeover` holds the loop, its status and schedule belong
		// to the human. A refusal is not an error — recovery requeues, normalizes
		// or parks loops, and none of those is the daemon's to make under a hold.
		// Nothing has been written yet when it happens, so the caller simply skips
		// the rest of this loop's recovery.
