...
func TestDecideRejectsForeignBareDispatchLabel(t *testing.T) {
	t.Parallel()

	decision := Decide(context.Background(), fixtureLLM{raw: `{"disposition":"valid","comment":"bad","labels":{"kind":["kind/bug"],"area":["area/coordinator"],"complexity":["complexity/m"],"dispatch":["dispatch/plan"]}}`}, Input{Issue: Issue{Title: "Bad", CreatedAt: time.Now().UTC().Format(time.RFC3339)}, Config: testConfig(), Now: time.Now().UTC()})
	if !decision.NoOp {
		t.Fatalf("Decide() = %#v, want no-op for foreign bare dispatch label", decision)
	}
}
