...
func TestClassify(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "unrecognized provider state is indeterminate", snapshot: PRSnapshot{PRNumber: 42, HeadSHA: "abc", AutoMergeEnabled: true, MergeableState: "future_state"}, budget: RetryBudget{Now: now, TransientRetries: 3, MaxIndeterminateDuration: 15 * time.Minute}, wantKind: ActionIndeterminate, wantUnknown: &now},
		...
		{name: "Mergify route remains watched without GitHub auto-merge", snapshot: PRSnapshot{PRNumber: 42, AutoMergeRouteEnabled: true, Mergeable: &mergeable, MergeableState: "clean"}, prior: &PriorWatchMarker{PRNumber: 42, HeadSHA: "abc"}, budget: RetryBudget{Now: now, TransientRetries: 3, MaxIndeterminateDuration: 15 * time.Minute}, wantKind: ActionStillPending},
