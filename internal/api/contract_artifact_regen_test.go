...
var responseArtifactOrder = []responseArtifactEntry{
	...
	{id: "events.list"},
	{id: "events.entity"},
	{id: "gatekeeper.agreements"},
	{id: "gatekeeper.verdicts"},
...
const loopLogsFollowDeclaredEntry = `{
  ...
  "notes": [
    ...
    "Event order: snapshot (once), zero or more chunk, then either end or error, then close.",
    ...
    "error data shape: { code: string, message: string, retryable: boolean, retryAfterMs?: number }; code is a daemon error code from the errors artifact's errorCodes list.",
    "error is emitted when a follow-mode poll, state refresh, or persisted-log read fails; the server writes it and closes the stream.",
    "retryable is true when the underlying failure maps to a 5xx status, and retryAfterMs is present (1000) only then."
  ],
  "events": {
    "snapshot": {
      "event": "snapshot",
    ...
    "error": {
      ...
      "data": {
        "code": "INTERNAL_ERROR",
        "message": "read loop logs: permission denied",
        "retryable": true,
        "retryAfterMs": 1000
      }
    }
...
func captureEventAndPullRequestResponses(t *testing.T, out map[string]capturedResponse) {
	...
	routes := []struct {
		...
		{id: "gatekeeper.agreements", path: "/api/v1/gatekeeper/agreements?limit=1"},
		{id: "gatekeeper.verdicts", path: "/api/v1/gatekeeper/verdicts?limit=1"},
