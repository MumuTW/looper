...
func TestHandlerProjectsCreateRouteSuccessDerivesDefaults(t *testing.T) {
	...
	NewHandler(Context{Config: fixture.config, Runtime: fixture.runtime, ProjectsService: fakeProjectService{
		addProject: func(context.Context, projects.AddInput) (projects.AddResult, error) {
			...
			return projects.AddResult{
				...
				Discovery: projects.DiscoveryState{Status: projects.DiscoveryStatusPending},
				Warnings:  nil,
			}, nil
		},
	}}).ServeHTTP(recorder, req)
	...
	if !ok {
		t.Fatalf("discovery = %#v, want object", data["discovery"])
	}
	assertEqual(t, discovery["status"], string(projects.DiscoveryStatusPending))
	...
	NewHandler(Context{Config: fixture.config, Runtime: fixture.runtime, ProjectsService: fakeProjectService{
		addProject: func(context.Context, projects.AddInput) (projects.AddResult, error) {
			...
			return projects.AddResult{
				...
				Repo:    &repo,
				Discovery: projects.DiscoveryState{
					Status:                 projects.DiscoveryStatusSucceeded,
...
func TestHandlerProjectsCreateRouteReturnsSuccessWhenWebhookRefreshFails(t *testing.T) {
	...
	assertEqual(t, data["archived"], false)
	if _, ok := data["discovery"].(map[string]any); !ok {
		t.Fatalf("discovery = %#v, want object", data["discovery"])
	}
	warnings, ok := data["warnings"].([]any)
	if !ok || len(warnings) != 0 {
...
func TestHandlerProjectDiscoverRouteRetriesDiscovery(t *testing.T) {
	fixture := newTestFixture(t)
	nowISO := fixture.now.UTC().Format(javaScriptISOString)
	...
	NewHandler(Context{Config: fixture.config, Runtime: fixture.runtime, ProjectsService: fakeProjectService{
		discoverProject: func(_ context.Context, input projects.DiscoverInput) (projects.DiscoverResult, error) {
			if input.ProjectID != "looper" {
				t.Fatalf("DiscoverProject().ProjectID = %q, want looper", input.ProjectID)
			}
...
func TestHandlerProjectDiscoverRouteRejectsGet(t *testing.T) {
	fixture := newTestFixture(t)
	req := httptest.NewRequest(http.MethodGet, "/api/v1/projects/looper/discover", nil)
	recorder := httptest.NewRecorder()

	NewHandler(Context{Config: fixture.config, Runtime: fixture.runtime, ProjectsService: fakeProjectService{}}).ServeHTTP(recorder, req)
...
func TestHandlerWorkerCreateRefusesIssueWithActiveFixer(t *testing.T) {
	fixture := newTestFixture(t)
	nowISO := fixture.now.UTC().Format(javaScriptISOString)
	metadata := `{"repo":"acme/looper","worktreeRoot":null,"source":"api"}`
	if err := fixture.runtime.Services().Repositories.Projects.Upsert(context.Background(), storage.ProjectRecord{ID: "project_1", Name: "Looper", RepoPath: "/tmp/repos/looper", MetadataJSON: &metadata, CreatedAt: nowISO, UpdatedAt: nowISO}); err != nil {
		t.Fatalf("Projects.Upsert() error = %v", err)
	}
	fixerLoopID := "loop_fixer_1"
	if err := fixture.runtime.Services().Repositories.Loops.Upsert(context.Background(), storage.LoopRecord{ID: fixerLoopID, Seq: 1, ProjectID: "project_1", Type: "fixer", TargetType: "issue", TargetID: stringPtr("issue:acme/looper:77"), Repo: stringPtr("acme/looper"), Status: "running", CreatedAt: nowISO, UpdatedAt: nowISO}); err != nil {
		t.Fatalf("Loops.Upsert() error = %v", err)
	}
	...
	body := parseJSONMap(t, recorder.Body.Bytes())
	errMap := body["error"].(map[string]any)
	assertEqual(t, errMap["code"], "LOOP_CONFLICT")
	assertEqual(t, errMap["message"], "Issue acme/looper#77 is already being worked by fixer (loop loop_fixer_1)")
}

func TestHandlerWorkerCreateAllowsIssueWithActivePlanner(t *testing.T) {
	fixture := newTestFixture(t)
	nowISO := fixture.now.UTC().Format(javaScriptISOString)
	metadata := `{"repo":"acme/looper","worktreeRoot":null,"source":"api"}`
	if err := fixture.runtime.Services().Repositories.Projects.Upsert(context.Background(), storage.ProjectRecord{ID: "project_1", Name: "Looper", RepoPath: "/tmp/repos/looper", MetadataJSON: &metadata, CreatedAt: nowISO, UpdatedAt: nowISO}); err != nil {
	...
	plannerLoopID := "loop_planner_1"
	if err := fixture.runtime.Services().Repositories.Loops.Upsert(context.Background(), storage.LoopRecord{ID: plannerLoopID, Seq: 1, ProjectID: "project_1", Type: "planner", TargetType: "issue", TargetID: stringPtr("issue:acme/looper:77"), Repo: stringPtr("acme/looper"), Status: "running", CreatedAt: nowISO, UpdatedAt: nowISO}); err != nil {
		t.Fatalf("Loops.Upsert() error = %v", err)
	}
	...
	NewHandler(Context{Config: fixture.config, Runtime: fixture.runtime, Now: func() time.Time { return fixture.now.Add(time.Minute) }}).ServeHTTP(recorder, req)
	if recorder.Code == http.StatusConflict {
		t.Fatalf("status = 409, want success; body=%s", recorder.Body.String())
	}
}
...
func (f fakeProjectService) DiscoverProject(ctx context.Context, input projects.DiscoverInput) (projects.DiscoverResult, error) {
	if f.discoverProject != nil {
		return f.discoverProject(ctx, input)
	}
	return projects.DiscoverResult{}, nil
}
