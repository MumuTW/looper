...
func TestUpgradeDrainClosesAdmissionAndReportsSupervisorSnapshot(t *testing.T) {
	...
	if started.Data.AdmissionState != string(looperdruntime.AdmissionDraining) {
		t.Fatalf("POST response = %#v, want admission draining", started.Data)
	}
	...
	// Producers are canceled on BeginDrain but may still be exiting; poll until
	// the snapshot reports drained rather than requiring it on the first POST.
	...
	for {
		...
		if time.Now().After(deadline) {
			t.Fatalf("GET never reported drained: %#v", observed.Data)
		}
