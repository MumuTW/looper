...
func (h *Handler) buildCreateLoopResponse(r *http.Request) (loopResponse, error) {
	...
	if domain.LoopType(loopType) == domain.LoopTypeWorker && target.TargetType != domain.LoopTargetTypeProject {
		metadataJSON, err = manualWorkerMetadataJSONCompat(metadataJSON, target, derefBool(body.Force))
		if err != nil {
			return loopResponse{}, err
		}
	}
	...
	record, err := storage.WithTransactionValue(r.Context(), services.Coordinator.DB(), nil, func(tx *sql.Tx) (storage.LoopRecord, error) {
		...
		upsertLoop := transactionRepos.Loops.Upsert
		if domain.LoopType(loopType) == domain.LoopTypeWorker && target.TargetType == domain.LoopTargetTypeIssue && derefBool(body.Force) {
			upsertLoop = transactionRepos.Loops.UpsertForcingIssueClaimAdmission
		}
		if err := upsertLoop(r.Context(), record); err != nil {
			return storage.LoopRecord{}, err
		}
...
func (h *Handler) buildWorkersCreateResponse(r *http.Request) (workerCreateResponse, error) {
	...
	workerPayload := struct {
		...
		IssueClaimOverride: derefBool(body.Force) && issueNumber != nil,
	}
	...
	record, err := storage.WithTransactionValue(r.Context(), services.Coordinator.DB(), nil, func(tx *sql.Tx) (storage.LoopRecord, error) {
		...
		if derefBool(body.Force) && issueNumber != nil {
			upsertLoop = repos.Loops.UpsertForcingIssueClaimAdmission
		}
		if upsertErr := upsertLoop(r.Context(), record); upsertErr != nil {
