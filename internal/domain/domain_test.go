...
func TestAssertStatusTransitions(t *testing.T) {
	...
	if err := AssertLoopStatusTransition(LoopStatusHumanTakeover, LoopStatusTerminated); err == nil {
		t.Fatal("AssertLoopStatusTransition(human_takeover, terminated) error = nil, want failure")
	}
	if err := AssertLoopStatusTransition(LoopStatusFailed, LoopStatusTerminated); err != nil {
		t.Fatalf("AssertLoopStatusTransition(failed, terminated) error = %v", err)
	}
...
func TestIsAutoLaneHeld(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "global blocks planner", loopType: LoopTypePlanner, labels: []string{labels.HoldGlobal}, want: true},
		{name: "planner ignores worker hold", loopType: LoopTypePlanner, labels: []string{labels.HoldWorker}, want: false},
		{name: "worker blocked by worker hold", loopType: LoopTypeWorker, labels: []string{labels.HoldWorker}, want: true},
