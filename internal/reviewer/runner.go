...
func (r *Runner) ProcessClaimedItem(ctx context.Context, queueItem storage.QueueItemRecord) (ProcessResult, error) {
	...
	// The run-start metadata is derived from the record updateLoop actually read,
	// not from the queued snapshot, so a policy published between enqueue and claim
	// is not overwritten. A derivation failure is fatal (origin/main's contract):
	// it is captured here and propagated after the write helper returns.
	...
	updatedLoop, held, err := r.beginLoopRun(ctx, *loop, func(updated *storage.LoopRecord) {
		...
		updated.NextRunAt = nil
		metadataJSON, err := r.recordLoopRunStartMetadata(updated.MetadataJSON, project.ID)
		if err != nil {
			metadataErr = err
			return
		}
	...
	if metadataErr != nil {
		return ProcessResult{}, metadataErr
	}
	if held {
