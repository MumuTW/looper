...
func (r *Runner) runPrepareWorktreeStep(ctx context.Context, input stepInput) (reviewerCheckpoint, error) {
	...
	if !prepared.Clean {
		...
		return checkpoint, &loopError{message: reviewerDirtyWorktreeMessage(branch, created.WorktreePath), kind: FailureManualIntervention}
	}
...
// reviewerDirtyWorktreeMessage keeps the failureclass "worktree is dirty" /
// "manual intervention required" markers while pointing operators at inspect
// and explicit retry paths. Never auto-discards.
func reviewerDirtyWorktreeMessage(branch, worktreePath string) string {
