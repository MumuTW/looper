...
func TestDecide(t *testing.T) {
	...
	basePR := PRSnapshot{Labels: []string{labels.DefaultWorkerReadyTrigger}, HasTrackedIssueLink: true}
	...
	tests := []struct {
		...
		{name: "mixed case looper label", pr: PRSnapshot{Labels: []string{strings.Replace(labels.DefaultWorkerReadyTrigger, "looper", "Looper", 1)}, HasTrackedIssueLink: true}, cfg: baseConfig, protection: baseProtection, settings: baseSettings, wantDecision: OptInWithStrategy(config.ReviewerAutoMergeStrategySquash)},
		{name: "label only", pr: PRSnapshot{Labels: []string{labels.DefaultWorkerReadyTrigger}}, cfg: baseConfig, protection: baseProtection, settings: baseSettings, wantDecision: RefuseWithReason(RefusalReasonScope)},
