...
func TestReviewProvenanceSeparatesReviewedRefusedAndAbsent(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			comments:      []githubinfra.CommentInfo{{ID: 77, Author: "coderabbitai[bot]", Body: "> [!NOTE]\n> Review limit reached. Additional reviews become available gradually."}},
		...
		},
		{
			...
			comments:      []githubinfra.CommentInfo{{ID: 78, Author: "coderabbitai[bot]", Body: "Review limit reached"}},
			wantStatus:    ReviewProvenanceReviewed,
			wantCompleted: 1,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := reviewProvenanceFrom(tt.reviews, tt.comments)
	...
	tests := []struct {
		...
		{name: "absent", mutate: func(*gatekeeperFixture) {}, wantStat: ReviewProvenanceAbsent},
		{
			...
			mutate: func(f *gatekeeperFixture) {
				f.github.comments = []githubinfra.CommentInfo{{ID: 9, Author: "coderabbitai[bot]", Body: "Review limit reached"}}
			},
...
func TestMergedPullRequestReportCarriesReviewProvenance(t *testing.T) {
	...
	fixture.github.comments = []githubinfra.CommentInfo{{ID: 5, Author: "coderabbitai[bot]", Body: "Review limit reached"}}
...
func TestListUnreviewedEnumeratesMergedPullRequestsWithoutACompletedReview(t *testing.T) {
	...
	seedGateReport(t, fixture, "acme/looper", 280, "MERGED", ReviewProvenance{
	...
	seedGateReport(t, fixture, "acme/looper", 281, "MERGED", ReviewProvenance{
	...
	seedGateReport(t, fixture, "acme/looper", 282, "MERGED", ReviewProvenance{Status: ReviewProvenanceUnknown}, fixture.now)
	// A report from a build that predates review provenance.
	seedGateReport(t, fixture, "acme/looper", 283, "MERGED", ReviewProvenance{Status: ReviewProvenanceUnobserved}, fixture.now)
	// Still open and unreviewed: real, but not a merged unreviewed change.
	seedGateReport(t, fixture, "acme/looper", 284, "OPEN", ReviewProvenance{Status: ReviewProvenanceAbsent}, fixture.now)
	// Two reports for one pull request: the later one is the current answer.
	seedGateReport(t, fixture, "acme/looper", 285, "MERGED", ReviewProvenance{Status: ReviewProvenanceAbsent}, fixture.now)
	seedGateReport(t, fixture, "acme/looper", 285, "MERGED", ReviewProvenance{
		Status: ReviewProvenanceReviewed, CompletedReviews: 1,
...
func seedGateReport(t *testing.T, fixture *gatekeeperFixture, repo string, prNumber int64, state string, provenance ReviewProvenance, at time.Time) {
	t.Helper()
	report := Report{
		Version: reportVersion, ProjectID: "project_1", Repo: repo, PRNumber: prNumber,
