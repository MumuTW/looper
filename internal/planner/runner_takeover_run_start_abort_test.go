...
// worktree creation and agent launch after the human was told the checkout is
...
func TestProcessClaimedItemAbortsWhenTakeoverWinsTheRunStartRace(t *testing.T) {
	...
	if err := fixture.repos.Loops.UpsertChangingHumanHold(ctx, storage.LoopRecord{
		ID: loopID, Seq: 42, ProjectID: projectID, Type: "planner",
	...
	queue := storage.QueueItemRecord{
		ID: "queue_takeover_run_start", ProjectID: &projectID, LoopID: &loopID, Type: "planner",
		TargetType: "issue", TargetID: target, Repo: &repo,
		DedupeKey: "planner:acme/looper:42", Priority: storage.QueuePriorityPlanner, Status: "running",
	...
	git := &fakeGitGateway{}
	github := &fakeGitHubGateway{issueDetail: IssueDetail{Number: issueNumber, Title: "Plan this", Body: "details"}}
	...
	// found nothing to do".
	if result.Summary != plannerRunStartHeldSummary {
		t.Fatalf("ProcessClaimedItem() summary = %q, want the run-start hold %q", result.Summary, plannerRunStartHeldSummary)
	}
	...
	if len(git.createCalls) != 0 || len(git.pushCalls) != 0 {
		t.Fatalf("git calls = create %#v, push %#v; want none after the hold was granted", git.createCalls, git.pushCalls)
	}
