...
func AppendFixerCompletionInstruction(prompt string) string {
	return strings.Join([]string{
		...
		`A blocked outcome must include a failure_kind describing what has to change before a retry could succeed:`,
		`- "retryable_transient": the obstacle is expected to clear on its own, such as a rate limit, a network or upstream service error, or a lock held by another process. The same attempt is worth repeating unchanged.`,
		`- "retryable_after_resume": the working state is wrong but recoverable, such as a stale checkout, a moved remote head, or a missing dependency that setup can install. A retry needs the environment re-prepared first, not just repeated.`,
