...
func TestProcessClaimedItemAbortsWhenTakeoverWinsTheRunStartRace(t *testing.T) {
	...
	target := buildIssueTargetID(repo, issueNumber)
	metadata := mustMarshalJSON(map[string]any{"worker": map[string]any{"title": "Implement worker loop", "repo": repo, "issueNumber": issueNumber, "baseBranch": "main", "executionMode": "create-pr"}})
	payload := mustMarshalJSON(map[string]any{"title": "Implement worker loop", "repo": repo, "issueNumber": issueNumber, "baseBranch": "main", "executionMode": "create-pr"})
	...
	if err := fixture.repos.Loops.UpsertChangingHumanHold(ctx, storage.LoopRecord{
		ID: loopID, Seq: 46, ProjectID: projectID, Type: "worker",
		TargetType: "issue", TargetID: &target, Repo: &repo,
	...
	queue := storage.QueueItemRecord{
		ID: "queue_takeover_run_start", ProjectID: &projectID, LoopID: &loopID, Type: "worker",
		TargetType: "issue", TargetID: target, Repo: &repo,
		DedupeKey: "worker:project_1:acme/looper:46", Priority: storage.QueuePriorityWorker, Status: "running",
	...
	git := &fakeGitGateway{}
	github := &fakeGitHubGateway{currentLogin: "octocat", issueDetail: IssueDetail{Number: issueNumber, Title: "Implement worker loop", Body: "details"}}
	...
	// found nothing to do".
	if result.Summary != workerRunStartHeldSummary {
		t.Fatalf("ProcessClaimedItem() summary = %q, want the run-start hold %q", result.Summary, workerRunStartHeldSummary)
	}
	...
	if len(git.createCalls) != 0 || len(git.restoreCalls) != 0 || len(git.prepareCalls) != 0 {
		t.Fatalf("git calls = create %#v, restore %#v, prepare %#v; want none after the hold was granted", git.createCalls, git.restoreCalls, git.prepareCalls)
	}
